...
void path_finder(int node_count, station_list_node* list_of_stations, struct Graph* graph, float dist[], int prev[], float new_dist[], float average[], float percent[], float delays[])
{
    ...
    printf("\nOverall Highest delay: %0.1f s", highest_num(highest_delays, node_count));
    printf("\nOverall Lowest delay: %0.1f s", lowest_num(lowest_delays, node_count));
    printf("\nOverall Median of averages: %0.1f s", median_finder(average, node_count));
    ...
    fprintf(output_file, "\nOverall highest delay: %0.1f s", highest_num(highest_delays, node_count));
    fprintf(output_file, "\nOverall lowest delay: %0.1f s", lowest_num(lowest_delays, node_count));
    fprintf(output_file, "\nOverall median of average delay: %0.1f s", median_finder(average, node_count));
...
void print_fine_Graph(struct Graph* graph, station_list_node* list_of_stations)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        while (ptr != NULL)
        {
            // strcat used to check string size of each station to properly align weights
