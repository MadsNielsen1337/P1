...
int train_match(struct Graph* graph, int search_pos)
{
    ...
    int match = segmented_string_compare(graph->head[search_pos+1]->allowed_trains, graph->head[search_pos]->allowed_trains);
    printf("\ngraph head: '%s'\n", graph->head[search_pos]->allowed_trains);
    printf("\ngraph head + 1: '%s'\n", graph->head[search_pos+1]->allowed_trains);
    printf("\nMATCHING: %d\n", match);
...
int dijkstra_recursive(struct Graph* graph, int search_pos, int target, int total_weight, int* p_recursion_nr, int* path){
    if (visited_position(search_pos, path)) {//detect if the algorithm tries to go back, meaning it has hit a dead end
        ...
        path[*p_recursion_nr] = search_pos;                //log current position in the path array where the index corresponds to the step number
        ...
        if(search_pos == target) {                         //check if we have reached the target and return 0 if we have, stopping the recursion
            ...
            int temp;
            while (ptr != NULL){
                *p_recursion_nr = *p_recursion_nr + 1;    //increment recursion_nr
                temp = dijkstra_recursive(graph, ptr->dest, target, total_weight, p_recursion_nr, path);

                if(temp != -1){                            //check if we have found an existing route with a smaller weight and replace the weight if we have
                    temp = graph->head[search_pos]->weight + temp + train_match(graph, search_pos);
...
int dijkstra(struct Graph* graph, int start_node, int end_node)
{
    ...
    int recursion_nr = 0;           //initialise integer to keep track of the number of recursions made
    int dijkstra_result = dijkstra_recursive(graph, start_node, end_node, 0, &recursion_nr, path);
